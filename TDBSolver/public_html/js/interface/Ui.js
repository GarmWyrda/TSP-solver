...
define('ui', ['listener', 'jQuery', 'logger', 'environment', 'map', 'way', 'distanceMatrix'], function(Listener, $, Logger, Environment, Map, Way) {
    var UI = {
        ...
        initialize: function() { //Cree tout les listeners de la page
            ...
            this.listeners.push(new Listener($("#getMatrixFly"), "click", Environment.getMatrixOffRoad));
            this.listeners.push(new Listener($("#getMatrixWalk"), "click", Environment.getMatrixByWalk));
        ...
        changePoints: function() { //Change les listes déroulantes en fonction du nombre de point affichés
            var $element = $("#list");
            Environment.nb = $element.val();
            UI.loadEmplacement(Environment.nb);
            Environment.clearMatrix();//on supprime la matrice en cours
